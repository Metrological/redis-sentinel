...
function Sentinel(name, endpoints, options) {
    ...
    // Redis client merge options (only used for server clients, not for sentinel clients).
    ...
    this.initSentinel = function (endpoint, callback) {
        ...
        var tempSentinel = this.createSentinelClient(endpoint, {retry_strategy: sentinelRetryStrategy});
        var tempPubSubRedis = this.createSentinelClient(endpoint, {retry_strategy: sentinelRetryStrategy});
    ...
    this.createSentinelClient = function (redisObj, overruleOptions, mergeOptions) {
        var self = this;
    ...
    this.log = function () {
        if (debug) {
            var params = ['redis-sentinel', clusterName].concat(Array.prototype.slice.call(arguments));
