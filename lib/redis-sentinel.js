...
function Sentinel(name, endpoints, options) {
    ...
    this.initSentinel = function (endpoint, callback) {
        ...
        var tempSentinel = this.createClient(endpoint, {connect_timeout: 7 * 24 * 3600 * 1000, retry_max_delay: 60000});
        var tempPubSubRedis = this.createClient(endpoint, {connect_timeout: 7 * 24 * 3600 * 1000, retry_max_delay: 60000});
        ...
        async.series(subs, function(err) {
            self.log('connection to sentinel ' + endpoint.host + ':' + endpoint.port, err);
            callback(null, !err);
        });
    };
    ...
    this.getActiveSentinel = function() {
        ...
        throw new Error('No active sentinels');
    };
    ...
    this.createClient = function (redisObj, overruleOptions, mergeOptions) {
        ...
        var options = overruleOptions || this.getRedisConnectionOptions();
        if (mergeOptions) {
            options = _.merge(mergeOptions, options);
        }
